...
type MockStub struct {
	...
	// History tracks every PutState call per key for GetHistoryForKey
...
func (stub *MockStub) PutState(key string, value []byte) error {
	...
	if stub.History == nil {
		stub.History = map[string][]*queryresult.KeyModification{}
	}
	stub.History[key] = append(stub.History[key], &queryresult.KeyModification{
		TxId:  stub.TxID,
		Value: value,
	})
...
func (stub *MockStub) GetHistoryForKey(key string) (shim.HistoryQueryIteratorInterface, error) {
	entries := stub.History[key]
	mods := make([]*queryresult.KeyModification, 0, len(entries))
	for _, e := range entries {
		mods = append(mods, e)
	}
	return &mockHistoryIterator{mods: mods}, nil
}
