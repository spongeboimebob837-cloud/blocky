...
ORG3_ADMIN_MSP="${TEST_NETWORK}/organizations/peerOrganizations/org3.example.com/users/Admin@org3.example.com/msp"

# Org3 crypto is regenerated by `network.sh up`/`down` (it rm -Rf's the whole
# peerOrganizations dir), so after any redeploy the org3 keys are gone. If they
# are missing, bring the org3 peer + crypto back up first (idempotent) rather
# than failing on a missing TLS cert below.
if [ ! -f "${ORG3_TLS}" ] || [ ! -d "${ORG3_ADMIN_MSP}" ]; then
  echo ">> Org3 crypto material not found (was the network redeployed?)"
  echo "   Running addOrg3/addOrg3.sh up to regenerate org3 peer + keys..."
  (cd "${ADD_ORG3}" && ./addOrg3.sh up)
fi
...
use_org() {
  ...
  case "${org}" in
    ...
    org3)
      ...
      export CORE_PEER_MSPCONFIGPATH="${ORG3_ADMIN_MSP}"
